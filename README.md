...
| Type | What it does |
|------|-------------|
| `job_income` | Sets monthly job income from a given age (zeroed at retirement) |
| `annual_bonus` | Adds a lump-sum cash bonus once per year until retirement, with optional `endAge` |
...
Post-retirement, if cash goes negative, the engine draws down investments to cover the shortfall.
...
**Job income vs. side income.** `job_income` events contribute to `monthlyJobIncome`, which is zeroed when a `retire` event fires. `side_income` events contribute to `monthlySideIncome` and keep flowing unless they have an explicit end age. This distinction matters for realistic post-retirement projections.
